...
{
    uint8_t flag = size / 65535;
    uint8_t flag_t = flag;
    while(flag)
    {
        int point = (flag_t - flag)*65535;
        HAL_SPI_Transmit(&hspi1, data+point, 65535, 100);
        flag -= 1;
    }
    HAL_SPI_Transmit(&hspi1, data+65535*flag_t, size-65535*flag_t, 100);
}
...
void LCD_ShowChar(uint16_t x, uint16_t y, char chr, uint8_t size)
{
    ...
    if((size == 16) || (size == 32) )	//16和32号字体
    {
        ...
        for(t = 0; t < csize; t++)
        {
            if(size == 16)temp = asc2_1608[(int)chr][t];	//调用1608字体
            else if(size == 32)temp = asc2_3216[(int)chr][t];	//调用3216字体
            else return;			//没有的字库
