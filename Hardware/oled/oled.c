...
{         	
	...
	temp2 = num - temp1;
	len1 = len - 6;//len1???????,?????????,????
	OLED_ShowChar(x + size2/2*4,y,'0'- 2,size2);//?????5??????
	x = x + size2/2;
	for(t=0;t<len1;t++)//???????
	{
	...
	OLED_ShowChar(x+(size2/2)*5,y,((int)(temp2*100)%10) + '0',size2); //???2?
	OLED_ShowChar(x+(size2/2)*6,y,((int)(temp2*1000)%10) + '0',size2); //???3?
	OLED_ShowChar(x+(size2/2)*7,y,((int)(temp2*10000)%10) + '0',size2); //???4?
}
